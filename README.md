...
* `docker run -it -p 8080:8080 -v $PWD:/swift-project -w /swift-project myapp-run sh -c .build-ubuntu/release/Untitled`


#### Kubernetes
To deploy your application to your Kubernetes cluster, run `helm install --name myapp .` in the `/chart/Untitled` directory. You need to make sure you change the `repository` variable in your `chart/Untitled/values.yaml` file points to the docker image containing your runnable application.
